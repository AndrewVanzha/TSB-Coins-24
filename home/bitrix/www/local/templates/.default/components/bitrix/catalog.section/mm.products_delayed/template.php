...
$catalogCoinsClasses = '';
$catalogCoinsClasses .= ($arParams["FULL_SIZE"] === 'Y' ? ' fullsize': '');
$catalogCoinsClasses .= (
    $arParams["DISPLAY_TABLE"] ? 
    $arParams["DISPLAY_TABLE"] :
    ($_COOKIE['view'] == "list" ? ' list' : ' grid')
);
$catalogCoinsClasses .= ($arParams["TOP_WRAPPER"] === 'Y' ? ' top-wrapper': '');

$basketProps = $arParams["BASKET_PARAMS"];

if(!function_exists('getUserDesired')) {
    ...
    <?
        foreach($arResult['ITEMS'] as $key => $arItem) {
	        ...
	        if (isset($pricesArray["RATIO_PRICE"])) {
		        $minPrice = $pricesArray["RATIO_PRICE"];
	        }
	        else {
		        $minPrice = $pricesArray["BASE_PRICE"];
	        }
	        $printPrice = $pricesArray["PRINT_PRICE"];
	        $printPrice_old = $pricesArray["PRINT_BASE_PRICE"];
	        $price = $pricesArray["RATIO_PRICE"];
	        $price_old = $pricesArray["BASE_PRICE"];
	        
	        $sale = false;
	        
	        // Лёня
	        $sale_percent = $pricesArray['PERCENT'];
	        if (($price != $price_old) && ($price > 0 || $sale_percent > 0)) $sale = true;
            
            $basketData = $basketProps[$arItem['ID']];
                        ...
                        <?if (count($aditionalImages) > 0):?>
                            ...
                            class="images__line-galery">
                                <div class="images__line-galery-items-wrapper">
                                    ...
                                    <?foreach ($aditionalImages as $adImg) {?>
                    ...
                    <div class="coin-item__description-wrapper">
                        ...
                        <div class="bottom-bar">
                            ...
                            <div class="basket-info">
                                <div class="basket-quant">
                                Количество: <span><?=$basketData['QUANTITY'];?> шт</span>
                                </div>
                                <div class="basket-price">
                                    Цена покупки: <span><?=$basketData['PRICE'];?></span>
                                </div>
                            </div>
                            <?}?>
                            <?if ($price > 0 && $arItem["PROPERTIES"]["AVAILABLE"]["VALUE"] == 1) {?>
                        ...
                        <? if ($arItem["PROPERTIES"]['ARTICLE']['VALUE'] == 52169060 || $arItem["PROPERTIES"]['ARTICLE']['VALUE'] == 52160060) : // монета 17.3.23  52150036? ?>
