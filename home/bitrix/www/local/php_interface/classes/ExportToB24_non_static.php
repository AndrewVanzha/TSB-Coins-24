...
{
	//URL вебхука	
	//define('URL','https://10.222.222.92/rest/1/udar3tjicfcv3pm6');
	private const URL = 'https://10.222.222.92/rest/1/udar3tjicfcv3pm6';	
	...
	self::log2file(" IS_NEW: " . $event->getParameter("IS_NEW"),'-start--IS_NEW');
		...
		{
			if (!empty($propertyItem->getField("CODE")) && gettype($propertyItem->getValue()) === "string")
			{
...
//===========================Добавление товаров в сделку======================================================================
	...
	private static function check_artikul ($articul) 
	{
	...
	private static function getAddressPickup($store_id)
	{
	...
	private static function getRegionFromAddress($address)
	{
	...
	private static function getRegionFromIp()
	{
		$is_bot = preg_match(
		 "~(Google|Yahoo|Rambler|Bot|Yandex|Spider|Snoopy|Crawler|Finder|Mail|curl)~i", 
		 $_SERVER['HTTP_USER_AGENT']
	...
	private static function getRegionFromPostIndex($postIndex)
	{
		$is_bot = preg_match(
		 "~(Google|Yahoo|Rambler|Bot|Yandex|Spider|Snoopy|Crawler|Finder|Mail|curl)~i", 
		 $_SERVER['HTTP_USER_AGENT']
	...
	//Запрос к REST API
	private static function executeHook($params, $url, $metod)
	{
	...
	private static function get_store($orderInfo)
	{
	 ...
	 * @return string
	 */
	private static function log2file($str, $fn = null, $folder=__DIR__."/logExportToB24/")
	{  
	...
	*/
	private function file_post_contents($url = 'https://10.222.222.92/app/importexport/getData.php', $data=array('key'=>"1q2w3e!QWE!",'art'=>'id'), $username = 'admin', $password = 'Kmr-bYp-CkW-sS2')
	{
		...
		*/
		...
		$u = 'https://10.222.222.92/app/importexport/getData.php?key=1q2w3e!QWE!&art=id';
		$url = 'https://10.222.222.92/app/importexport/getData.php';
		$data = array(
			'key'=>'1q2w3e!QWE!',
			'art'=>'id'
		);
		...
		//return json_decode(file_get_contents('http://10.222.222.92/app/importexport/getData.php?key=1q2w3e!QWE!&art=id', false), true);
	} 
		// Транслитерация строк.
	private static function translit($str, $s = "")
	{
